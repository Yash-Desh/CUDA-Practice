...
// ####################### READ ME #################################
// This file shows a method to do the exclusive scan. 
// It uses the double buffering optimization for Kogge-Stone Method
// #################################################################
...
// Exclusive scan (prefix sum) on the CPU.
...
// Exclusive scan (prefix sum) on the GPU. 
__global__ void scan_kernel(float* input, float* output, float* partialSums, unsigned int N) {
    ...
    // Pointers to help identify the input & output buffer
    // for that iteration. 
    ...
    // Shift data by 1 as it is an exclusive scan. 
    if(threadIdx.x == 0) {
    ...
    // Perform inclusive scan on the shifted array.
